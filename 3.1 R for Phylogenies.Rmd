...
# NZ instead of plotting anything, it's giving a help window letting me know what is going on with plots
...
# NZ It seems that each command gives information on how to understand the organization of the tree based on the N code that was entered to create it along with the read.tree and plot commands; we get attributes such as edge, node and tip, (and clade?) that it is a phylogeny, the amount of items in the list, how many nodes and tips (which seems so, so handy); it seems like this is concrete information about the list but also the list's organizational principles. So I see that we have three internal nodes, though I'm unsure how to understand the $ edge information from str(tr_primates) right away. It also tells me that it's organized/ordered by clade, etc. 
...
#Deskobjects fake tree:
```{r}
#literally objects on my desk. 

newick_str = "(Zamioculcas,(Crackers, Chocolate Bar, Whiskey));"
tr_deskobjects = read.tree(text=newick_str)
plot(tr_deskobjects)
```
...
#Cabbage is a Brassicae, corn is a grass, kale is a Brassicae, and Wheat is a grass. So the tree is wrong, because Kale and Cabbage should be in the same grouping () in this tree, not Cabbage and Corn. 
...
#I think it is easier to see in this format, but will be even better with branch lengths included, because that to me is more interesting to see (though I know just the relationships are also important)
...
#In this tree the Poaceae are closer together, but corn is older than wheat, so I think it should share a node with wheat but it'd be more accurate if it were switched with wheat? 
...
#erroneous tree (plus two)
```{r}
newick_str = "(Wheat,(Corn, Radish, Sequoia, (Cabbage, Kale)));" 
...
#tree (plus two) in a hopefully better order
```{r}
newick_str = "(Sequoia,(Wheat, Corn, (Cabbage, Radish, Kale)));"
tr5_plants = read.tree(text=newick_str)
plot(tr5_plants)

str(tr5_plants)
attributes(tr5_plants)
```


a. Explain what's wrong in your erroneous tree and how you've corrected the error in the other tree. 

# a.the erroneous tree lumps sequoia, radishes and corn together leaving wheat as an outgroup. I attempted to make a better tree by placing sequoia in an outgroup, because wheat and corn are both grasses and more closely related to one another, and radishes, cabbage and kale are all mustards and should be grouped together in this tree. 
...
# The str function gives $ tip.label: chr [1:6], which shows the tip names in the order I gave, so they differ there. The attributes are all the same and the other str function information is the same.
...
newick_str = "(((Humans:12.5, Chimps:12.5):1.0, Gorillas:14.0):1.0, (Pongo abelii:3.5, Pongo tapanuliensis:3.5)):1.0;"
tr_primate6 = read.tree(text=newick_str)
plot(tr_primate6)
```
...
layout(matrix(1:4, 2, 2))
plot(tr5_plants, type="cladogram")
plot(tr5_plants, type="fan")
plot(tr5_plants, type="unrooted")
plot(tr5_plants, type="radial")

```

```{r}
layout(matrix(1:4, 2, 2))
plot(tr5_plants, main = "normal", type = "phylogram", direction = c("upwards"))
plot(ladderize(tr5_plants, FALSE), main = "left-ladderized", direction=c("upwards"))
layout(matrix(1, 1))
...
plot(tr5_plants, type="unrooted", edge.width=5, edge.color="green", lab4ut="horizontal")
...
newick_fn = "tr5_plants"
write.tree(tr5_plants, file=newick_fn)
```
...
```{r}
pdf_fn = "tr5_plants.pdf"
pdf(file=pdf_fn)
plot(tr5_plants, type="unrooted", edge.width=5, edge.color="green", lab4ut="axial")
dev.off()
```
...
#better for many reasons - capitalizations (mostly) and in slightly different order, and more importantly shows the branch lengths
```{r}
...
#wrong below at first! because taxa is run on a different variable. So you need to run it on spp1
has n-1 tips becase 1 taxa could not be matched, could be misspelled

#NOT misspelled (Abies_bornmuelleriana), so maybe just not in the stored tree?


```{r}
spp1 <- c("Abies_nordmanniana", "Abies bornmuelleriana", "Abies_cilicica", "Abies_cephalonica",
"Abies_numidica", "Abies_pinsapo", "Abies_alba", "Ginkgo biloba", "Nymphaea lotus", "Illicium mexicanum", "Magnolia fraseri","Lindera pulcherrima")
tree_phylomatic <- phylomatic(taxa = spp1, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic, no.margin = TRUE)
```
...
Here's an example of this type of data phylogenized: lists of species from Emily Gregory's anatomy textbook. using global names resolver, couldhave saved as a text file
...
#two different versions named differently
...
#marchantia - is not an angiosperm and so not in her tree - so some of the things like selaginella, a seedless vascular plant is not in the reference genome - could be put in as an outgroup if wanted though - but are not in the tree because "not matched" like all bacteria, fungi, cryptogams, seedless/nonvascular plants in her file but not for this tree
...
#this one is my own slashpath file for the assignment:
```{r}
plansla <- read.table("planto slashpath copy.txt")
#You can find this file in your working directory and look at how it is set up. 
#Note that it already assigns every species and/or genus to a family
#When you pass in species names or genus names, the phylomatic function automatically uses an R Package TAXIZE to look up and associate the taxon names with the family, and then to format the data to feed it into the phylomatic command this way. 
#Take a look at the output from the line immediately below to inspect the data and how its set up.
plansla
tree_plantomatic_PHY1 <- phylomatic(taxa = plansla)
#We have dropped the Get =POST command.
plot(tree_plantomatic_PHY1, no.margin = TRUE)
```


#plot tree_plantomatic_PHY1 in new ways

```{r}


#save as newick
newick_planto1 = "tree_plantomatic_PHY1"
write.tree(tree_plantomatic_PHY1, file=newick_planto1)

newick_fn4 = "tree_plantomatic_PHY1"
write.tree(tree_plantomatic_PHY1, file=newick_fn4)


pdf_fn1 ="tree_plantomatic_PHY1"
pdf(file=pdf_fn1)
```



# The first time I tried the tree from my queens plant list below, it told me "3 taxa not matched: NA/chlorophyta/chlorophyta, NA/equisetum/equisetum_arvense, NA/licopus/licopus_amplectens" - licopus was misspelled to their spelling, so I resolved that, chlorophyta is algae, so it wouldn't be in their database, but I'm unsure why equisetum arvense, field horsetail, would not get mapped (unless it's just plain not in there), but as far as I can tell the herb is spelled right. 

```{r}
taxaqueens <-c("Acer negundo", "Acer rubrum", "Acer saccharinum", "Acer saccharum", "Ailanthus altissima", "Betula lenta", "Brasenia schreberi", "Callitriche heterophylla", "Carya cordiformis", "Carya glabra", "Carya tomentosa", "Cephalanthus occidentalis", "Chlorophyta", "Clethra alnifolia", "Decodon verticillatus", "Distichlis spicata", "Eleocharis obtusa", "Equisetum arvense", "Eubotrys racemosa", "Fagus grandifolia", "Fraxinus americana", "Geranium maculatum", "Hypericum mutilum", "Iva frutescens", "Juncus acuminatus", "Juglans nigra", "Juncus effusus", "Lycopus amplectens", "Lemna", "Liquidambar styraciflua", "Liriodendron tulipifera", "Lycopus rubellus")

taxaqueens

queenstree_phylomatic <- phylomatic(taxa = taxaqueens, storedtree ="R20120829", get = 'POST')
plot(queenstree_phylomatic, no.margin = TRUE)

queenstree_phylomatic <- phylomatic(taxa = taxaqueens, storedtree ="zanne2014", get = 'POST')
plot(queenstree_phylomatic, no.margin = TRUE)

plot(queenstree_phylomatic, type="cladogram")

pdf_fn = "queenstree_phylomatic.pdf"
pdf(file=pdf_fn)
plot(queenstree_phylomatic, type="unrooted", edge.width=5, edge.color="green", lab4ut="axial")
dev.off()

newick_fn4 = "queenstree_phylomatic"
write.tree(queenstree_phylomatic, file=newick_fn4)


```


#too small for this many plants:
```{r}
layout(matrix(1:4, 2, 2))
plot(tree_plantomatic_PHY1, type="cladogram")
plot(tree_plantomatic_PHY1, type="fan")
plot(tree_plantomatic_PHY1, type="unrooted")
plot(tree_plantomatic_PHY1, type="radial")
```

#The below might not be useful to read, but I think it looks pretty cool? IT looks like a piece of seaweed. Did this with both trees.

```{r}
plot(queenstree_phylomatic, type="cladogram")
plot(tree_plantomatic_PHY1, type="unrooted", edge.width=5, edge.color="dark green", lab4ut="axial")
...
#I'm taking this question in part to mean the difference between the cladograms and the phylograms. The cladograms are, in some ways, easier to read and prettier--but it only shows relationships between the species. They don't show the time scale between species or how much evolutionary change there may have been over time, just the common ancestor that exists going back toward the root. The phylograms are more to reflect the time scale between species based on the distance between the species and nodes via branch length. Also, the stored trees don't always have the species that I'd like to plot, it seems, because not every tree/newick code file will have absolutely everything in the universe included within it, so the species I put into my tree may be missing an ancestory or piece of information that could shange some of the organization of these trees. 
...
#In one case, there was an algae included in my tree (for queens), which was not indcluded in the list. In another case, though, I couldn't figure out why Equisetum arvense wasn't plotted; likely it was just not in the database I drew from. 
...
#(Will be DONE by the time it's read)
